...
export type FeatureFlags = {
  /** TMDB film search and import is configured (TMDB_API_KEY set). */
  tmdb: boolean;
  /** BoardGameGeek board-game search and import is configured (BGG_API_TOKEN set). */
  bgg: boolean;
  /** Unsplash image search is configured (UNSPLASH_ACCESS_KEY set). */
  unsplash: boolean;
};

export function PersonPageClient({ features }: { features: FeatureFlags }) {
  ...
  // External search is only "available" when both the category supports it
  // AND the corresponding API key is configured. When the key is missing the
  // category falls back to manual-only adds, mirroring lege/kreative/andre.
  const isExternalCategory =
    (selectedCat?.slug === "film" && features.tmdb) ||
    (selectedCat?.slug === "braetspil" && features.bgg);
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      {manualOpen && selectedCat && (
        <AddActivityForm
          ...
          imageSearchEnabled={features.unsplash}
